...
//funcions 
    ...
    public void initialize(URL url, ResourceBundle rb) {
	  ...
	  dateexchange_datepicker.setPromptText(" اختر التاريخ من هنا ");
	  ...
	  Tooltip tip = new Tooltip(" اختر اسم الألة ");
    ...
    public boolean getValidation(){
		...
		while (true){
			if (!nbon_txt_.matches("[0-9]+")){
			 formErrors[0] = " الرجاء ادخال رقم البون";
			    ...
			    try{
					...
					while(rs.next()){
						...
						if (nbon_distict == nbon_check){
						    String concat = "  البون الذى تحاول ادخاله مكرر فى المسلسل رقم  " + retrive_serialn_of_distinct;
					        setAlert(AlertType.INFORMATION, "خطأ فادح","راجع البيانات جيدا",concat);
							no_distinct = false;
						} 
			...
			if (quantitybon_txt_.matches("[0-9]+")){
				if (Integer.valueOf(quantitybon_txt_) > 254 || Integer.valueOf(quantitybon_txt_) < 0){
					...
					formErrors[1] = "الارقام تكون من 1 الى 254 فى السعه";
					valid = false;
			    ...
			    formErrors[2] = "ارقام فقط فى خانه السعه ";
			    valid = false;
			}
			
			if (!gas_radiobtn.isSelected() && !solar_radiobtn.isSelected()){
			 
			 formErrors[3] = "اختر نوع الوقود";
			 valid = false;
			}else{
			    formErrors[3] = null;
			}
			...
			if (!counter_txt_.matches("[0-9]+")){
			 ...
			 formErrors[4] = "ادخل رقم العداد ارقام فقط";
			 valid = false;
			}else{
			  formErrors[4] = null;
			}
			...
			if (!nnote_txt_.matches("[0-9]+")){
			 ...
			 formErrors[5] = " ادخل رقم الدفتر رقم فقط ";
			 valid = false;
			}else{
			formErrors[5] = null;
			}
			...
			if (!codemachine_txt_.matches("[0-9]+")){
			 formErrors[6] = "ادخل كود الألة ارقام فقط";
			 valid = false;		
			}else{
			    formErrors[6] = null;
			}
			...
			if (date_ == null){
			  formErrors[9] = "اختر التاريخ من القائمه";
			  valid = false;
			}else{
			    formErrors[9] = null;
			}
			if (nmachine.equals("")){
				formErrors[10] =  "اختر نوع الألة";
				valid = false;
			}else{
			   formErrors[10] = null;
			}
		...
		codemachine_val = nmachine +" "+codemachine_txt_+"ك";
   ...
   @FXML //this way isn't correct way , the correct way using by Group and toggle classes
	...
	public void setTextFG(){
		...
		if (type_fuel.equals("بنزين")){
			gas_radiobtn.setSelected(true);
			solar_radiobtn.setSelected(false);
		}else if (type_fuel.equals("سولار")){
